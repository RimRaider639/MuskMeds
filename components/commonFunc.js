...
//base urls
const PRODUCT_URL = "https://lapis-quixotic-dracorex.glitch.me/products?";
const CART_URL = "https://lapis-quixotic-dracorex.glitch.me/cart/";
...
const init = () => {
  ...
  if (localStorage.getItem("loggedIn") == "true") {
    ...
    document.getElementById("login-signup").innerHTML =
      "<div id='user'><span>" +
      user.name +
      "</span><span id=signOut>Sign Out</span></div>";
...
const createButtons = (baseURL, totalResults, funcArray) => {
  ...
  for (let i = 1; i <= num; i++) {
    ...
    if (i == 1) {
      ...
      next.onclick = () =>
        document.querySelector(`#nav_btn>button:nth-child(${2})`).click();
    }
    btn.onclick = async () => {
      ...
      if (i == 1) {
        ...
        prev.onclick = () =>
          document.querySelector(`#nav_btn>button:nth-child(${i - 1})`).click();
      }
      if (i == num) {
...
const unhideButtons = (totalPages) => {
  ...
  for (let i = 1; i <= MAX_BUTTONS + Math.max(0, current - MAX_BUTTONS); i++) {
    document
      .querySelector(`#nav_btn>button:nth-child(${i})`)
    ...
    data.forEach((prod) => {
      if (prod[attr]) {
        let found = labels.find(
          (l) => l.name.toLowerCase() == prod[attr].toLowerCase()
        );
        if (found) {
          found.incCount();
        } else {
...
const appendProducts = (data, selector) => {
  ...
  data.forEach((prod) => {
    ...
    title.innerText =
      t.length > 9 ? t.slice(0, MAX_WORDS).join(" ") + "..." : prod.Name;
    ...
    rating.innerHTML =
      "<span>" +
      prod.rating +
      "<span id='fill' class='material-symbols-outlined'>star</span></span> " +
      prod.rating_count;
...
const appendLabel = (labelTypeObj, selector, baseURL) => {
  let data = labelTypeObj.filtered
    ? labelTypeObj.filtered
    : labelTypeObj.labels;
